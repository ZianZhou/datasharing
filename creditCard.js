...
const idInvalidCardCompanies=(nestArray)=>{
  ...
  let fraudComps = [];
  for(let i=0;i<nestArray.length;i++){
    if(nestArray[i][0]===3 && !foundAmex){
      fraudComps.push('Amex');
      foundAmex=true;
    } else if(nestArray[i][0]===4 &&!foundVisa){
      fraudComps.push('Visa');
      foundVisa=true;
    } else if(nestArray[i][0]===5 && !foundMastercard){
      fraudComps.push('Mastercard');
      foundMastercard=true;
    } else if(nestArray[i][0]===6 && !foundDiscover){
      fraudComps.push('Discover');
      foundDiscover=true;
    } else {
